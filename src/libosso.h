...
#define LIBOSSO_API_MAJOR 2
#define LIBOSSO_API_MINOR 21
...
/**
 * This function initializes the library, using the D-Bus connections passed
 * as arguments. Notice that dbus_connection_setup_with_g_main is not called
 * for the D-Bus connections, allowing for the use of a custom main loop.
 * Using a private (from dbus_bus_get_private) connection is also possible.
 * #osso_initialize_with_connections should be called
 * only once by the application.
 * @param application The name of the application.
 * This name forms the last part of the default (D-Bus) service name of the
 * application. Note that the D-Bus service name will be
 ...
 * @param version The version string of the application. It must be 
 * comparable with strcmp().
 * @param system_conn D-Bus system bus connection.
 * @param session_conn D-Bus session bus connection.
 * @return A context to use in later calls to this library. NULL is
 * returned if an error happened.
 */
